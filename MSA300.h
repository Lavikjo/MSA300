...
/*=========================================================================
    I2C ADDRESS/BITS
    -----------------------------------------------------------------------*/
    #define MSA300_I2C_ADDRESS_WRITE                 (0x4C)    // Assumes SDO pulled to GND, otherwise 0x4E
    #define MSA300_I2C_ADDRESS_READ                  (0x4D)    // Assumes SDO pulled to GND, otherwise 0x4F
/*=========================================================================*/
...
/*=========================================================================
    ...
    #define MSA300_REG_ACC_Z_LSB            (0x02) // X-acceleration LSB (R)
    #define MSA300_REG_ACC_Z_MSB            (0x03) // X-acceleration MSB (R)
    ...
    #define MSA300_REG_OFFSET_COMP_Z        (0x40) // Z Offset Compensation (R/W)
...
/* Used with register 0x10 (MSA300_REG_ODR) to set datarate and with register 0x11 (MSA_REG_PWR_MODE_BW) to set Bandwidth */
typedef enum
{
...
/* Used with register 0x0F (MSA300_REG_RES_RANGE) to set g range */
typedef enum
{
...
typedef enum
{
  MSA300_INT_NON_LATCHED      = 0b0000,
  MSA300_INT_LATCHED_250_MS   = 0b0001,
  MSA300_INT_LATCHED_500_MS   = 0b0010,
  MSA300_INT_LATCHED_1_S      = 0b0011,
  MSA300_INT_LATCHED_2_S      = 0b0100,
  MSA300_INT_LATCHED_4_S      = 0b0101,
  MSA300_INT_LATCHED_8_S      = 0b0110,
  MSA300_INT_LATCHED          = 0b0111,
  MSA300_INT_LATCHED_1_MS     = 0b1001,
  MSA300_INT_LATCHED_2_MS     = 0b1011,
  MSA300_INT_LATCHED_25_MS    = 0b1100,
  MSA300_INT_LATCHED_50_MS    = 0b1101,
  MSA300_INT_LATCHED_100_MS   = 0b1110
} intMode_t;
...
typedef enum
{
  MSA300_TAP_DUR_50_MS        = 0b000,
  MSA300_TAP_DUR_100_MS       = 0b001,
  MSA300_TAP_DUR_150_MS       = 0b010,
  MSA300_TAP_DUR_200_MS       = 0b011,
  MSA300_TAP_DUR_250_MS       = 0b100,
  MSA300_TAP_DUR_375_MS       = 0b101,
  MSA300_TAP_DUR_500_MS       = 0b110,
  MSA300_TAP_DUR_700_MS       = 0b111
} tapDuration_t;
...
typedef struct
{
  bool orientInt = false;
  bool sTapInt = false;
  bool dTapInt = false;
  bool activeInt = false;
  bool freefallInt = false;
  bool newDataInt = false;
  ...
  union 
  {
    struct 
    {
      ...
      uint8_t tapFirstZ;
      uint8_t activeSign;
      uint8_t activeFirstX;
      uint8_t activeFirstY;
      uint8_t activeFirstZ;
    } intStatus;
...
typedef enum 
{
  ORIENT_UPWARD_LOOKING       = 0b0,
  ORIENT_DOWNWARD_LOOKING     = 0b1
} zOrient_t;

typedef enum 
{
  ORIENT_PORTRAIT_UPRIGHT     = 0b00,
  ORIENT_PORTRAIT_UPSIDEDOWN  = 0b01,
  ORIENT_LANDSCAPE_LEFT       = 0b10,
  ORIENT_LANDSCAPE_RIGHT      = 0b11
} xyOrient_t;
...
typedef enum
{
  MODE_SYMMETRICAL            = 0b00,
  MODE_HIGH_ASYMMETRICAL      = 0b01,
  MODE_LOW_ASYMMETRICAL       = 0b10
} orientMode_t;

typedef enum
{
  ORIENT_NO_BLOCKING          = 0b00,
  ORIENT_Z_BLOCKING           = 0b01,
  ORIENT_Z_BLOCKING_0_2_G     = 0b10
} orientBlockMode_t;
...
class MSA300{
 public:
  ...
  MSA300(uint8_t clock, uint8_t miso, uint8_t mosi, uint8_t cs, int32_t sensorID = -1);
  ...
  void        setTapDuration(tapDuration_t duration, uin8_t quiet, uint8_t shock);
  ...
  void        setFreefallHysteresis(float value);
